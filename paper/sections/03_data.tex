...
Our system draws on five data sources: the official FIFA Fantasy API
...
fantasy scoring \citep{vaastav}, and a static copy of the FIFA Men's
World Ranking as a fallback. This section describes each source, the
schemas we consume, and the harmonisation work required to join them.
...
\POSTFINAL{number: labelled player-gameweek training rows across the
three seasons after dropping zero-minute rows} labelled rows across
the three seasons.
