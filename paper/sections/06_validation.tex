...
roughly \POSTFINAL{number: pre-determinism per-position RMSE
run-to-run variation} between runs, driven by LightGBM's bagging
...
per-round traces. Four configurations are compared.
...
\item \textbf{D: EPL plus WC, all features.} As C, plus a lagged
  start-rate feature and a team goals-conceded form feature.
\end{itemize}
...
worse (\WalkforwardPooledD{}). We deployed C: it wins overall and at
the two attacking positions that dominate squad decisions. On the
round-of-16 feature set the retrained model ranks the elite in-form
forwards, including Mbapp\'e, Kane, and Haaland, at the top of its
