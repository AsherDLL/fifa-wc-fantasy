...
reversed in the knockout rounds and the reversal persisted through
the semi-finals (Table~\ref{tab:backtest-rounds}), with cumulative
totals of \TotalHeuristicVTwo{} for v2 against \TotalHeuristic{} for
v1: the component stack we rejected on a three-round gate ended the
replay ahead of the baseline it lost to. We stand by the decision
procedure while reporting the outcome honestly; a three-round gate
is noisy in both directions, and the same gate discipline is what
caught the genuinely harmful candidates below. The procedural
lesson stood regardless and became policy:
...
(\AvailabilityWithout{} to \AvailabilityWith{} on the heuristic
backend, MD1 to R32, measured at deployment time) and was stable
...
A coda arrived before the final round and sharpens the diagnosis.
When real lineup sheets became available through the community match
dataset (Section~\ref{sec:validation-walkforward}, configuration F),
we replaced the noisy points-based proxy with true lagged start
rates and minutes shares, testing whether the failure above was the
proxy or the signal. Real minutes did better than the proxy, beating
the deployed configuration C at \WalkforwardPooledF{} against
\WalkforwardPooledC{}, but only marginally: the margin fell under
our pre-registered deploy threshold's caution band, forwards
regressed, and the real-xG configuration E evaluated in the same run
was strictly better at every position. So even the \emph{true}
minutes signal earns no place in the point model, and the
conclusion of this subsection survives with a finer grain: minutes
information is real, it belongs in the selection layer, and in the
model layer team-level scoring form dominates player-level
participation.
...
carry: the log implied win probabilities correlate at \MarketEloCorr{}
with our derived country Elo (across the 27 countries priced at the
first archived snapshot),
