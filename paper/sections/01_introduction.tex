...
overrode the model, and whether the override was vindicated. The
entry closed the tournament on \TotalUserActualNet{} net points over
the eight rounds, finishing at global rank \GlobalRankFinal{} among
the game's worldwide entries; its highest-scoring round
(\FinalRoundNet{} points) was the final one, on a squad
restructured by model-recommended transfers into the eventual
champion's defensive block.
...
\item \textbf{Open source and data.} The full codebase, frozen data
  ...
  (\url{https://github.com/AsherDLL/fifa-wc-fantasy}).
\end{enumerate}
