...
is leak-free throughout: every model change faced a walk-forward gate
on realized tournament rounds before deployment, and we report the
failures alongside the successes. Across the seven completed rounds
...
entry closed on \TotalUserActualNet{} net points and global rank
\GlobalRankFinal{}, saving its best round for last: the locked
final-round plan, six-strong in the eventual champion's defence with
the qualification booster active, returned \FinalRoundNet{} points
and gained roughly a quarter-million global ranks in a single round.
The complete system, data snapshots, and decision log are released
as open source.
\end{abstract}
...
\input{sections/07_live_results}
\input{sections/08_analysis}
\input{sections/09_negative_results}
\input{sections/10_lessons}
